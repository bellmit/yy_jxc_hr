package nc.ui.jxc_hr.jx04.sgd_add.action;
 ...
 * @author duy
 */
public class CopyActionProcessor implements ICopyActionProcessor<SGDVO_ADD> {

  @Override
  public void processVOAfterCopy(SGDVO_ADD billVO, LoginContext context) {
    this.processHeadVO(billVO, context);
    this.processBodyVO(billVO);
  }

  private void processBodyVO(SGDVO_ADD vo) {
  ...
  private void processHeadVO(SGDVO_ADD vo, LoginContext context) {
    UFDateTime datetime = ServerTimeProxy.getInstance().getServerTime();
    SGDHVO_ADD hvo = vo.getParentVO();
    // 设置空处理
    hvo.setSgdno(null);
