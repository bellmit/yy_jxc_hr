package nc.ui.jxc_hr.ht01.htgl.action;
 ...
 * @author duy
 */
public class CopyActionProcessor implements ICopyActionProcessor<HTVO> {

  @Override
  public void processVOAfterCopy(HTVO billVO, LoginContext context) {
    this.processHeadVO(billVO, context);
    this.processBodyVO(billVO);
  }

  private void processBodyVO(HTVO vo) {
  ...
  private void processHeadVO(HTVO vo, LoginContext context) {
    UFDateTime datetime = ServerTimeProxy.getInstance().getServerTime();
    HtHVO hvo = vo.getParentVO();
    // 设置空处理
    hvo.setBillno(null);
