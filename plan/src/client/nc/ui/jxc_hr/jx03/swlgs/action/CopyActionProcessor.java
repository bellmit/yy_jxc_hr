 ...
 * @author duy
 */
public class CopyActionProcessor implements ICopyActionProcessor<SWLGSVO> {

	@Override
	public void processVOAfterCopy(SWLGSVO billVO, LoginContext context) {
		this.processHeadVO(billVO, context);
		this.processBodyVO(billVO);
	}

	private void processBodyVO(SWLGSVO vo) {
	...
	private void processHeadVO(SWLGSVO vo, LoginContext context) {
		UFDateTime datetime = ServerTimeProxy.getInstance().getServerTime();
		SWLGSHVO hvo = vo.getParentVO();
		// 设置空处理
		hvo.setBillno(null);
