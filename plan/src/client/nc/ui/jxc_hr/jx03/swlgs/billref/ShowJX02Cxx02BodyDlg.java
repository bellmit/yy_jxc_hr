 ...
 * 
 */
public class ShowJX02Cxx02BodyDlg extends UIDialog implements ActionListener,
		BillEditListener2 {
	// 界面主Panel
	...
	public ShowJX02Cxx02BodyDlg(Container parent, PGDBVO01[] bvos) {
		super(parent);
		initialize();
		setDefaultData(bvos);
		try {
	...
	private HashMap<String, PGDHVO> getPGDHvoMap(PGDBVO01[] bvos) {
		...
		String sql = "select * from jxc_hr_pgk_h  where nvl(dr,0)=0  and "    //加一个交易类型PK检验区分临时任务书和派工卡
				+ inStr;
	...
	public void actionPerformed(ActionEvent e) {
		try {
			if (e.getSource().equals(getBtnCancel())) {
				...
				SWLGSVO[] destVos = (SWLGSVO[]) tr.VOtransfer("JX02-Cxx-02", "JX03",
